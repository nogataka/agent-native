...
{
  "tone": "success",
  "body": "**Sicher zu aktivieren, sicher wiederherzustellen.** Identitätsschreibvorgänge sind **nur additiv** – ein vorhandenes Konto mit derselben E-Mail-Adresse wird unverändert wiederverwendet und eine neue E-Mail fügt lediglich eine neue Zeile ein. Es gibt keine Schemaänderung und es muss nichts migriert werden, sodass das Ein- oder Ausschalten von `AGENT_NATIVE_IDENTITY_HUB_URL` jederzeit und pro App vollständig rückgängig gemacht werden kann."
}
```
