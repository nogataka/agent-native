...
{
  "html": "<div class=\"diagram-ladder\"><div class=\"diagram-card rung rung-3\"><span class=\"diagram-pill accent\">단계 3 · agent-native</span><strong>에이전트 + UI가 동등한 파트너</strong><small class=\"diagram-muted\">One action surface. Every agent tool is also a button; every button runs the same logic the agent uses.</small></div><div class=\"diagram-card rung rung-2\"><span class=\"diagram-pill\">단계 2</span><strong>도구가 있는 채팅</strong><small class=\"diagram-muted\">에이전트는 동작할 수 있지만 여전히 채팅 창일 뿐입니다. 대시보드, 목록, 단축키는 없습니다.</small></div><div class=\"diagram-card rung rung-1\"><span class=\"diagram-pill warn\">단계 1</span><strong>단일 LLM 호출</strong><small class=\"diagram-muted\">Prompt in, string out. Impressive in a demo; breaks the moment reality gets messy.</small></div></div>",
...
{
  "html": "<div class=\"diagram-parity\"><div class=\"diagram-col\"><div class=\"diagram-node\">Human<br><small class=\"diagram-muted\">클릭, 폼, 단축키</small></div><div class=\"diagram-node\">Agent<br><small class=\"diagram-muted\">자연어 · A2A · Slack</small></div></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-panel center\"><span class=\"diagram-pill accent\">Actions</span><small class=\"diagram-muted\">defined once</small></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-box\">SQL 데이터베이스</div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&#8635;</div><div class=\"diagram-box\">UI가 실시간 업데이트</div></div>",
...
```an-annotated-code title="한 번 정의된 하나의 작업"
{
  ...
  "annotations": [
    { "lines": "5", "label": "도구 표면", "note": "`description`는 이를 도구로 호출할 시기를 결정하기 위해 에이전트가 읽는 것입니다." },
    { "lines": "6", "label": "타입 계약", "note": "하나의 zod `schema`가 에이전트, UI, HTTP, MCP, A2A 등 **모든** 표면의 입력을 검증합니다." },
    { "lines": "7-10", "label": "하나의 구현", "note": "`run` 본문은 단일 정보 소스입니다. UI 버튼과 에이전트 도구 모두 이를 정확하게 실행합니다." }
  ]
}
```
