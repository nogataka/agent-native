...
{
  "surface": "desktop",
  "html": "<div style='display:flex;flex-direction:column;gap:14px;padding:18px;min-height:520px;box-sizing:border-box'><div style='display:flex;align-items:center;gap:10px'><h1 style='margin:0'>출시 브랜드</h1><span class='wf-pill accent'>블로그 hero</span><span class='wf-pill'>제품 이미지</span><span class='wf-pill'>Logos</span><div style='flex:1'></div><button>Upload</button><button class='primary'>Generate</button></div><div class='wf-card' style='display:flex;flex-direction:column;gap:10px'><strong>브랜드 미디어 만들기</strong><div class='wf-box'>승인된 로고와 제품 참고 자료를 사용한 홈페이지 hero 옵션 3개.</div><div style='display:flex;gap:8px;flex-wrap:wrap'><span class='wf-pill accent'>4 references</span><span class='wf-pill'>16:9</span><span class='wf-pill'>웹 내보내기</span></div></div><div style='display:grid;grid-template-columns:repeat(3,1fr);gap:12px;flex:1'><div class='wf-card' style='display:flex;align-items:end;min-height:130px'><span class='wf-pill accent'>히어로 A</span></div><div class='wf-card' style='display:flex;align-items:end;min-height:130px'><span class='wf-pill'>참조 세트</span></div><div class='wf-card' style='display:flex;align-items:end;min-height:130px'><span class='wf-pill'>로고 안전</span></div></div><div class='wf-card' style='display:grid;grid-template-columns:repeat(4,1fr);gap:8px'><div class='wf-box'>Use</div><div class='wf-box'>Refine</div><div class='wf-box'>Compare</div><div class='wf-box'>Export</div></div></div>"
}
```
...
{
  "html": "<div class=\"diagram-assets\"><div class=\"diagram-col\"><div class=\"diagram-node\">References<br><small class=\"diagram-muted\">로고, 제품 이미지, 스타일</small></div><div class=\"diagram-node\">프롬프트<br><small class=\"diagram-muted\">채팅 또는 Generate 컨트롤</small></div></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-box\" data-rough><span class=\"diagram-pill accent\">생성 세션</span><small class=\"diagram-muted\">image &amp; video candidates · audit log</small></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-box\" data-rough><span class=\"diagram-pill ok\">Library</span><small class=\"diagram-muted\">선택된 브랜드 일관 assets</small></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-col\"><div class=\"diagram-node\">Picker<br><small class=\"diagram-muted\">iframe / MCP 앱</small></div><div class=\"diagram-node\">A2A<br><small class=\"diagram-muted\">슬라이드 · 디자인 · 콘텐츠</small></div></div></div>",
...
```an-schema title="자산 데이터 모델" summary="Libraries are the ownable container; collections, folders, and presets organize them. Sessions drive generate-and-choose; assets and runs hold output and the audit log. Table names keep the legacy image_* prefix but cover all media."
{
  "entities": [
    { "id": "library", "name": "image_libraries", "note": "최상위 소유 가능 컨테이너", "fields": [
    ...
    { "id": "library_shares", "name": "image_library_shares", "note": "프레임워크 공유 테이블", "fields": [
      { "name": "library_id", "type": "id", "fk": "image_libraries.id" },
      { "name": "role", "type": "text", "note": "viewer / editor / admin" }
    ] },
    { "id": "collections", "name": "image_collections", "note": "Style/category 그룹화", "fields": [
      { "name": "library_id", "type": "id", "fk": "image_libraries.id" },
      { "name": "style_brief", "type": "text", "nullable": true },
      { "name": "prompt_template", "type": "text", "nullable": true }
    ] },
    { "id": "folders", "name": "asset_folders", "note": "중첩 가능한 폴더", "fields": [
      { "name": "library_id", "type": "id", "fk": "image_libraries.id" },
      { "name": "parent_id", "type": "id", "fk": "asset_folders.id", "nullable": true }
    ] },
    { "id": "presets", "name": "image_generation_presets", "note": "저장된 세대 레시피", "fields": [
      { "name": "media_type", "type": "text" },
      { "name": "prompt_template", "type": "text" },
      { "name": "model", "type": "text" }
    ] },
    { "id": "sessions", "name": "image_generation_sessions", "note": "반복적인 생성 및 선택", "fields": [
      { "name": "id", "type": "id", "pk": true },
      { "name": "status", "type": "text" },
      { "name": "active_asset_id", "type": "id", "fk": "image_assets.id", "nullable": true }
    ] },
    { "id": "session_items", "name": "image_generation_session_items", "note": "세션의 후보 자산", "fields": [
      { "name": "session_id", "type": "id", "fk": "image_generation_sessions.id" },
      { "name": "asset_id", "type": "id", "fk": "image_assets.id" },
      { "name": "role", "type": "text" }
    ] },
    { "id": "assets", "name": "image_assets", "note": "자산 기록", "fields": [
      { "name": "id", "type": "id", "pk": true },
    ...
    { "id": "runs", "name": "image_generation_runs", "note": "생성 감사 로그", "fields": [
      { "name": "model", "type": "text" },
      { "name": "status", "type": "text" },
...
```bash
# 가장 쉬운 호스팅 설치: 내보낸 기술 지침과 MCP 커넥터.
npx @agent-native/core@latest skills add assets

# Vercel/open 스킬 CLI 설치: 지침만 내보냈으며 MCP 구성은 없습니다.
npx skills@latest add BuilderIO/agent-native --skill assets

# 호스팅된 설치: URL 전용 MCP 커넥터, 기술 파일에 공유 비밀이 없습니다.
