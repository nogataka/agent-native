...
{
  "html": "<div class=\"ws-map\"><div class=\"diagram-card cc\"><span class=\"diagram-pill warn\">Claude Code / Codex</span><small class=\"diagram-muted\">~/.claude/ on a local disk</small><div class=\"ws-files\"><span class=\"diagram-box\">CLAUDE.md</span><span class=\"diagram-box\">skills/</span><span class=\"diagram-box\">memory</span><span class=\"diagram-box\">mcp.json</span></div><small class=\"diagram-muted\">one codebase per developer</small></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-card an\"><span class=\"diagram-pill accent\">Agent-native 워크스페이스</span><small class=\"diagram-muted\">rows in one SQL 데이터베이스</small><div class=\"ws-rows\"><span class=\"diagram-pill\">AGENTS.md</span><span class=\"diagram-pill\">skills/&hellip;</span><span class=\"diagram-pill\">memory/&hellip;</span><span class=\"diagram-pill\">mcp-servers/&hellip;</span></div><small class=\"diagram-muted\">하나의 코드베이스, 여러 사용자, 범위 지정 <code>u:&lt;email&gt;:&hellip;</code></small></div></div>",
...
{
  "html": "<div class=\"ws-stack\"><div class=\"diagram-card\"><span class=\"diagram-pill\">Workspace</span><small class=\"diagram-muted\">Dispatch의 회사 전체 기본값</small><code>context/brand.md</code></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&darr;</div><div class=\"diagram-card\"><span class=\"diagram-pill\">조직 / 앱</span><small class=\"diagram-muted\">team override for one app</small><code>context/brand.md</code></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&darr;</div><div class=\"diagram-card\"><span class=\"diagram-pill accent\">Personal</span><small class=\"diagram-muted\">per-user override &mdash; wins</small><code>context/brand.md</code></div><div class=\"diagram-arrow diagram-accent\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-box ok\">Effective <code>context/brand.md</code></div></div>",
...
```an-callout
{ "tone": "info", "body": "다시 시작하거나 재배포할 필요가 없습니다. `AGENTS.md`은 매 턴 시작 시 읽혀지므로 지금 저장하는 편집 내용은 바로 다음 메시지에서 에이전트의 행동을 변경합니다." }
```
...
```text
context/company.md              # 회사가 하는 일, ICP, 제품, 링크
context/brand.md                # 음성, 시각적 정체성, 철자법, 금지된 사용
context/messaging.md            # 포지셔닝, 가치 제안, 입증 포인트, 반대 의견
instructions/guardrails.md      # 규정 준수, 에스컬레이션 및 승인 규칙
...
```an-schema title="작업공간 자원 모델" summary="하나의 리소스 형태는 모든 작업 공간 파일을 백업합니다. 런타임은 이를 경로 및 범위별로 키를 지정하고 읽기 시 유효 값을 확인합니다."
{
  "entities": [
    {
      "id": "resource",
      "name": "workspace resource",
      "note": "사용자 작업 공간의 단일 파일(명령어, 스킬, 메모리, 에이전트, MCP 구성 또는 작업)입니다.",
      "fields": [
        { "name": "path", "type": "string", "note": "Canonical path, e.g. AGENTS.md, skills/<slug>/SKILL.md" },
        { "name": "scope", "type": "workspace | shared | personal", "note": "이 행은 어느 수준에 있나요?" },
        { "name": "owner", "type": "string", "nullable": true, "note": "u:<email> for personal scope" },
        { "name": "content", "type": "text", "note": "마크다운 / JSON / YAML 본문" }
      ]
    }
  ]
}
...
```an-annotated-code title="맞춤형 상담원 프로필"
{
  ...
  "language": "markdown",
  "code": "---\nname: Design\ndescription: >-\n  Reviews layouts, interaction patterns, and product UX decisions.\nmodel: inherit\ntools: inherit\ndelegate-default: false\n---\n\n# 역할\n\n당신은 집중적인 디자인 에이전트입니다.\n\n## 책임\n\n- 레이아웃 및 상호 작용 흐름 검토\n- 더 강력한 시각적 방향 제안\n- 간결하고 자기 주장이 강합니다.",
  "annotations": [
    { "lines": "2", "label": "@mention handle", "note": "`name`은 `@`-드롭다운에 나타나는 내용이며 주 에이전트가 위임하는 내용입니다." },
    { "lines": "3-4", "label": "위임할 시기", "note": "`description`은 이 프로필이 작업에 적합한지 결정하기 위해 오케스트레이터가 읽는 것입니다." },
    { "lines": "5", "label": "모델", "note": "`inherit`는 주 에이전트의 모델을 재사용합니다. 프로필에 분명히 다른 프로필이 필요한 경우에만 재정의하세요." },
    { "lines": "6", "note": "현재 `tools: inherit` — 이 필드는 향후 에이전트별 도구 정책을 위해 예약되어 있습니다." }
  ]
}
```
...
# Dispatch가 관리하는 상속된 작업공간 컨텍스트 읽기
...
# 리소스 작성
pnpm action resource-write --path "notes/meeting.md" --content "# 회의록..."
