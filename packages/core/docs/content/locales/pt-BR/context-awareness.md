...
{
  "html": "<div class=\"diagram-ctx\"><div class=\"diagram-card col\"><span class=\"diagram-pill\">UI escreve</span><div class=\"diagram-node\">navigation<br><small class=\"diagram-muted\">visualização, IDs abertos</small></div><div class=\"diagram-node\">__url__<br><small class=\"diagram-muted\">shareable filters</small></div><div class=\"diagram-node\">selection<br><small class=\"diagram-muted\">linhas, blocos, formas</small></div></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-panel center\" data-rough><span class=\"diagram-pill accent\">view-screen</span><small class=\"diagram-muted\">reads state &middot; fetches records</small></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-box\">Agente age<br><small class=\"diagram-muted\">on the real object</small></div><div class=\"diagram-arrow diagram-accent\" aria-hidden=\"true\">&#8635;</div><div class=\"diagram-box diagram-accent\">navigate<br><small class=\"diagram-muted\">agente move a UI</small></div></div>",
...
```an-annotated-code title="view-screen — os olhos do agente"
{
  ...
  "annotations": [
    { "lines": "10-11", "label": "Superfície da ferramenta", "note": "O agente lê esta descrição para saber que pode chamar `view-screen` para ver o UI atual." },
    { "lines": "13", "label": "http: false", "note": "Ação interna — não exposta em HTTP. O agente e `pnpm action` chamam isso, não o navegador." },
    { "lines": "15-16", "label": "Estado de leitura", "note": "Extrai as chaves leves `navigation` e `selection` que UI escreveu." },
    { "lines": "23-37", "label": "Hidratar", "note": "Transforma esses IDs em registros **frescos** direto de SQL, para que o agente verifique o objeto ativo antes de agir." }
  ]
}
```
...
{
  "tone": "info",
  "body": "**Mantenha `navigation` e `selection` pequenos.** Armazene IDs mais rótulos curtos, não registros inteiros. `view-screen` busca a fonte da verdade sob demanda, para que o estado obsoleto ou volumoso nunca chegue ao agente."
}
```
...
{
  "html": "<div class=\"diagram-jitter\"><div class=\"diagram-node\">Esta aba escreve<br><small class=\"diagram-muted\">X-Request-Source: TAB_ID</small></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-box\" data-rough>Servidor armazena origem<br>on the event</div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-card col\"><div class=\"diagram-pill warn\">source == TAB_ID &rarr; ignored</div><small class=\"diagram-muted\">sem refetch, sem piscar</small><div class=\"diagram-pill ok\">agent / other tab &rarr; applied</div><small class=\"diagram-muted\">A UI atualiza ao vivo</small></div></div>",
