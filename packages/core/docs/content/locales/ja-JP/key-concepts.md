...
>
> **アプリケーション** — エージェントの周囲の製品表面。これは、最初はアクションのみ、リッチ チャット、小さなコントロール プレーン、またはダッシュボード、フロー、ビジュアライゼーションを備えた完全な React UI画面 である可能性があります。
>
...
{
  "html": "<div class=\"diagram-arch\"><div class=\"diagram-row\"><div class=\"diagram-card\"><span class=\"diagram-pill accent\">Agent</span><small class=\"diagram-muted\">データを読み書きし、actionsを実行し、コードを変更</small></div><div class=\"diagram-card\"><span class=\"diagram-pill\">Application</span><small class=\"diagram-muted\">actionのみ、チャット、コントロールプレーン、または完全なReact UI画面</small></div></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&darr;&nbsp;&uarr;</div><div class=\"diagram-box\" data-rough>Computer<br><small class=\"diagram-muted\">SQLデータベース · browser · code execution</small></div></div>",
...
```an-schema title="コア SQL ストア" summary="すべてのテンプレートで自動作成されます。エージェントと UI は両方ともこれらの読み取りと書き込みを行います。"
{
  "entities": [
    { "id": "application_state", "name": "application_state", "note": "エージェントがコンテキストのために読み取る一時的な UI 状態", "fields": [
      { "name": "key", "type": "text", "pk": true, "note": "e.g。 「ナビゲーション」" },
      { "name": "value", "type": "json", "note": "ビュー、選択、ドラフト" }
    ] },
    { "id": "settings", "name": "settings", "note": "永続的なキーと値の構成", "fields": [
      { "name": "key", "type": "text", "pk": true },
      { "name": "value", "type": "json" }
    ] },
    { "id": "oauth_tokens", "name": "oauth_tokens", "note": "OAuth認証情報", "fields": [
      { "name": "provider", "type": "text", "pk": true },
      { "name": "token", "type": "text" }
    ] },
    { "id": "sessions", "name": "sessions", "note": "認証セッション", "fields": [
...
```bash
# 迅速なデータベース検査と 1 回限りのメンテナンスのための主要なアクション
...
# 大きなテキスト列の Surgical find/replace — 値全体ではなく差分を送信します
...
{
  "html": "<div class=\"diagram-sync\"><div class=\"diagram-node\">エージェントアクション<br><small class=\"diagram-muted\">DB に書き込み</small></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-node\">変更イベント<br><small class=\"diagram-muted\">source: action / settings</small></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-panel center\"><span class=\"diagram-pill accent\">useDbSync</span><small class=\"diagram-muted\">SSE &middot; poll fallback</small></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-node\">クエリ再取得<br><small class=\"diagram-muted\">再読み込みなしで描画</small></div></div>",
...
- **BYO エージェント チャット ランタイム。** 同じチャット UI は、OpenAI エージェント、OpenAI レスポンス、Claude エージェント SDK、Vercel AI SDK、AG-UI、または独自の正規化された HTTP ストリームの上に置くことができます。 [Native チャット UI](/docs/native-chat-ui#byo-agent-runtimes) を参照してください。
...
- [Native チャット UI](/docs/native-chat-ui) — アクション宣言されたテーブル、チャート、および BYO 実行時の状態
