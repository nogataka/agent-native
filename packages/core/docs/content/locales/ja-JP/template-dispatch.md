...
{
  "surface": "desktop",
  "html": "<div style='display:flex;flex-direction:column;gap:14px;padding:18px;min-height:520px;box-sizing:border-box'><div style='display:flex;align-items:center;gap:10px'><h1 style='margin:0'>Dispatch</h1><span class='wf-pill accent'>Overview</span><span class='wf-pill'>Inbox</span><span class='wf-pill'>Secrets</span><span class='wf-pill'>Approvals</span><div style='flex:1'></div><button>Schedules</button></div><div class='wf-card' style='display:flex;flex-direction:column;gap:10px'><strong>次に何をしますか？</strong><div class='wf-box'>Analyticsに今週の登録数を聞き、Slack更新を下書きする。</div><button class='primary'>Delegate</button></div><div style='display:grid;grid-template-columns:repeat(3,1fr);gap:10px'><div class='wf-card'><strong>Mail</strong><br/><small>/mail</small></div><div class='wf-card'><strong>Calendar</strong><br/><small>/calendar</small></div><div class='wf-card'><strong>Analytics</strong><br/><small>/analytics</small></div><div class='wf-card'><strong>Slides</strong><br/><small>/slides</small></div><div class='wf-card'><strong>Forms</strong><br/><small>/forms</small></div><div class='wf-card'><strong>アプリを作成</strong><br/><small>+</small></div></div><div class='wf-card' style='display:grid;grid-template-columns:repeat(3,1fr);gap:8px'><div class='wf-box'>SlackのDM に返信が必要</div><div class='wf-box'>A2Aタスク完了</div><div class='wf-box'>承認が必要</div></div></div>"
}
```
...
{
  "html": "<div class=\"diagram-dispatch\"><div class=\"diagram-col\"><div class=\"diagram-node\">Slack · Telegram</div><div class=\"diagram-node\">Email</div><div class=\"diagram-node\">A2Aリクエスト</div></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-box\" data-rough><span class=\"diagram-pill accent\">Orchestrator</span><small class=\"diagram-muted\">中央受信箱 · トリアージ · ルーティング</small></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-col\"><div class=\"diagram-node\">Mail エージェント</div><div class=\"diagram-node\">Analytics エージェント</div><div class=\"diagram-node\">Brain · Slides &hellip;</div></div></div><div class=\"diagram-shared\"><span class=\"diagram-pill\">シークレット保管庫</span><span class=\"diagram-pill\">ワークスペースリソース</span><span class=\"diagram-pill warn\">Approvals</span><span class=\"diagram-pill\">スケジュール済みジョブ</span></div>",
...
- **中央受信箱。** SlackのDM、電報メッセージ、電子メール通知、他のエージェントからの A2A リクエストはすべて 1 か所に届きます。 Dispatch エージェントはトリアージを行い、それ自体または代理でそれらを処理します。 Slack、電子メール、テレグラムをワークスペースに接続する方法については、[Messaging](/docs/messaging) を参照してください。
...
- **シークレット保管庫.** API キー、OAuth トークン、および共有資格情報の中央ストア。ワークスペース内のアプリは、すべての `.env` でシークレットを複製するのではなく、Dispatch からシークレットを解決します。機密性の高いアクセスに対するリクエストと承認。
...
- **メッセージを適切な専門家にルーティングします。** 分析に関する SlackのDM は分析エージェントに送信されます。電子メールに関するものはメール エージェント、つまりディスパッチ ピックに送られます。
...
```an-schema title="Secrets ボールトのスキーマ" summary="Secrets are stored once; grants give a named app access; requests + reviews gate sensitive access; the audit log records who used which secret when. Defined in @agent-native/dispatch (packages/dispatch/src/db/schema.ts)."
{
  "entities": [
    { "id": "secrets", "name": "vault_secrets", "note": "保存された資格情報の値", "fields": [
    ...
    { "id": "grants", "name": "vault_grants", "note": "アプリごとのアクセス許可", "fields": [
      { "name": "id", "type": "text", "pk": true },
    ...
    { "id": "requests", "name": "vault_requests", "note": "アクセスリクエスト+レビュー", "fields": [
      { "name": "id", "type": "text", "pk": true },
      { "name": "credential_key", "type": "text" },
      { "name": "app_id", "type": "text" },
    ...
    { "id": "audit", "name": "vault_audit_log", "note": "誰がいつどのシークレットを使用したか", "fields": [
      { "name": "id", "type": "text", "pk": true },
...
# 複数選択ピッカーで「Dispatch」と、必要なドメイン アプリを選択します
```
...
Dispatch は、他のテンプレートと同様に完全なテンプレートです。[Templates](/docs/cloneable-saas) を参照してください。エージェントに「Datadog の新しい統合を追加」または「チャネル X から分析エージェントに SlackのDM をルーティング」するよう依頼すると、ルーティング構成が編集され、Webhook ハンドラーが追加され、接続されます。
