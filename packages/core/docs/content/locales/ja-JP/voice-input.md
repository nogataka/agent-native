...
{
  "html": "<div class=\"diagram-voice\"><div class=\"diagram-node\">マイクボタン<br><small class=\"diagram-muted\">webm/opus を録音</small></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-card col\"><div class=\"diagram-pill accent\">1 &middot; Builder Gemini</div><small class=\"diagram-muted\">Builder 接続時のデフォルト</small><div class=\"diagram-pill\">2 &middot; BYOK cloud</div><small class=\"diagram-muted\">Gemini &middot; Groq &middot; OpenAI Whisper</small></div><div class=\"diagram-arrow diagram-warn\" aria-hidden=\"true\">&darr;</div><div class=\"diagram-box diagram-warn\" data-rough>3 &middot; Browser Web Speech<br><small class=\"diagram-muted\">fallback on 400 &middot; streams live</small></div></div>",
...
{
  "tone": "info",
  "body": "**認証情報の解決順序:**ルートは最初にユーザー自身の暗号化された秘密を確認し、次に共有展開キーを確認します。独自のキーを持つパワー ユーザーは、常に共有キーをオーバーライドします。どちらも存在しない場合、ルートはコンポーザーが認識する 400 を返し、黙ってブラウザの Web Speech にフォールバックします。"
}
```

## ルート {#route}

```an-api title="音声転写ルート"
{
  "method": "POST",
  "path": "/_agent-native/transcribe-voice",
  "summary": "録音されたオーディオクリップをプロンプトテキストに書き起こす",
