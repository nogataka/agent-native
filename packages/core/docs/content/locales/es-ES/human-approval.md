...
```an-annotated-code title="Cerrar la única acción consecuente"
{
  ...
  "annotations": [
    { "lines": "10", "label": "toda la puerta", "note": "Una bandera. Si es veraz y la llamada no está aprobada, el ciclo se detiene antes de `run`: el modelo nunca alcanza el efecto secundario por sí solo." },
    { "lines": "11-13", "label": "run() is untouched", "note": "El manejador sigue siendo el mismo. La aprobación la impone el bucle que lo rodea, no nada dentro de `run`." }
  ]
}
```
...
{
  "html": "<div class=\"diagram-approve\"><div class=\"diagram-box\" data-rough>El agente llama a send-email</div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&darr;</div><div class=\"diagram-panel warn\" data-rough><strong>Gate verdadero, llamada aún no aprobada</strong><small class=\"diagram-muted\">loop emits tool_start + approval_required { tool, input, approvalKey }</small><span class=\"diagram-pill warn\">turn pauses &mdash; run() did NOT execute</span></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&darr;</div><div class=\"diagram-box\" data-rough>La persona hace clic en Aprobar en el chat<br><small class=\"diagram-muted\">el cliente reenvía el turno con approvedToolCalls: [approvalKey]</small></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&darr;</div><div class=\"diagram-panel ok\" data-rough><span class=\"diagram-pill ok\">Gate sees the key &rarr; run() executes &rarr; email sends</span></div></div>",
