...
{
  "html": "<div class=\"ws-map\"><div class=\"diagram-card cc\"><span class=\"diagram-pill warn\">Claude Code / Codex</span><small class=\"diagram-muted\">~/.claude/ on a local disk</small><div class=\"ws-files\"><span class=\"diagram-box\">CLAUDE.md</span><span class=\"diagram-box\">skills/</span><span class=\"diagram-box\">memory</span><span class=\"diagram-box\">mcp.json</span></div><small class=\"diagram-muted\">one codebase per developer</small></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-card an\"><span class=\"diagram-pill accent\">مساحة عمل Agent-native</span><small class=\"diagram-muted\">rows in one قاعدة بيانات SQL</small><div class=\"ws-rows\"><span class=\"diagram-pill\">AGENTS.md</span><span class=\"diagram-pill\">skills/&hellip;</span><span class=\"diagram-pill\">memory/&hellip;</span><span class=\"diagram-pill\">mcp-servers/&hellip;</span></div><small class=\"diagram-muted\">قاعدة كود واحدة، مستخدمون متعددون، بنطاقات <code>u:&lt;email&gt;:&hellip;</code></small></div></div>",
...
{
  "html": "<div class=\"ws-stack\"><div class=\"diagram-card\"><span class=\"diagram-pill\">Workspace</span><small class=\"diagram-muted\">إعدادات افتراضية على مستوى الشركة من Dispatch</small><code>context/brand.md</code></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&darr;</div><div class=\"diagram-card\"><span class=\"diagram-pill\">المؤسسة / التطبيق</span><small class=\"diagram-muted\">team override for one app</small><code>context/brand.md</code></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&darr;</div><div class=\"diagram-card\"><span class=\"diagram-pill accent\">Personal</span><small class=\"diagram-muted\">per-user override &mdash; wins</small><code>context/brand.md</code></div><div class=\"diagram-arrow diagram-accent\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-box ok\">Effective <code>context/brand.md</code></div></div>",
...
```an-callout
{ "tone": "info", "body": "لا إعادة تشغيل ولا إعادة نشر. تتم قراءة `AGENTS.md` في بداية كل دورة، لذا فإن التعديل الذي تحفظه الآن يغير سلوك الوكيل في الرسالة التالية." }
```
...
```an-schema title="نموذج موارد مساحة العمل" summary="يدعم شكل مورد واحد كل ملف مساحة عمل. يقوم وقت التشغيل بمفاتيحه حسب المسار والنطاق ويحل القيمة الفعالة عند القراءة."
{
  "entities": [
    {
      "id": "resource",
      ...
      "fields": [
        ...
        { "name": "scope", "type": "workspace | shared | personal", "note": "في أي مستوى يعيش هذا الصف" },
        { "name": "owner", "type": "string", "nullable": true, "note": "u:<email> for personal scope" },
        { "name": "content", "type": "text", "note": "Markdown / JSON / YAML الجسم" }
      ]
    }
  ]
}
...
```an-annotated-code title="ملف تعريف وكيل مخصص"
{
  ...
  "annotations": [
    ...
    { "lines": "5", "label": "نموذج", "note": "`inherit` يعيد استخدام نموذج الوكيل الرئيسي. يتم التجاوز فقط عندما يحتاج ملف التعريف بوضوح إلى ملف تعريف مختلف." },
    { "lines": "6", "note": "`tools: inherit` في الوقت الحالي — الحقل محجوز لسياسات الأدوات المستقبلية لكل وكيل." }
  ]
}
```
