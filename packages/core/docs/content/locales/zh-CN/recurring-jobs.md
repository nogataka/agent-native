...
```an-annotated-code title="jobs/morning-digest.md"
{
  ...
  "language": "markdown",
  "code": "---\nschedule: \"0 7 * * *\"\nenabled: true\nrunAs: creator\n---\n\n# 早间摘要\n\n总结隔夜收到的电子邮件。按发件人域分组。\n将今天看起来需要回复的前 3 个线程固定到\n\"Needs reply\" 标签。对任何显而易见的问题草拟答复。",
  "annotations": [
    { "lines": "2", "label": "当", "note": "标准 5 字段 cron — `0 7 * * *` 是每天 07:00。" },
    { "lines": "3", "label": "暂停开关", "note": "Flip to `false` to stop the job without deleting it." },
    { "lines": "4", "label": "身份", "note": "`creator` runs with the owner's identity and `ANTHROPIC_API_KEY`; `shared` uses the org's key." },
    { "lines": "7-12", "label": "提示", "note": "主体只是一个提示 - 代理在每次触发时使用其所有常规工具和工作区上下文运行它。" }
  ]
}
```
