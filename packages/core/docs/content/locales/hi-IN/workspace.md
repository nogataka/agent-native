...
{
  "html": "<div class=\"ws-map\"><div class=\"diagram-card cc\"><span class=\"diagram-pill warn\">Claude Code / Codex</span><small class=\"diagram-muted\">~/.claude/ on a local disk</small><div class=\"ws-files\"><span class=\"diagram-box\">CLAUDE.md</span><span class=\"diagram-box\">skills/</span><span class=\"diagram-box\">memory</span><span class=\"diagram-box\">mcp.json</span></div><small class=\"diagram-muted\">one codebase per developer</small></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-card an\"><span class=\"diagram-pill accent\">Agent-native कार्यक्षेत्र</span><small class=\"diagram-muted\">rows in one SQL डेटाबेस</small><div class=\"ws-rows\"><span class=\"diagram-pill\">AGENTS.md</span><span class=\"diagram-pill\">skills/&hellip;</span><span class=\"diagram-pill\">memory/&hellip;</span><span class=\"diagram-pill\">mcp-servers/&hellip;</span></div><small class=\"diagram-muted\">एक codebase, कई users, scoped <code>u:&lt;email&gt;:&hellip;</code></small></div></div>",
...
{
  "html": "<div class=\"ws-stack\"><div class=\"diagram-card\"><span class=\"diagram-pill\">Workspace</span><small class=\"diagram-muted\">Dispatch से कंपनी-व्यापी defaults</small><code>context/brand.md</code></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&darr;</div><div class=\"diagram-card\"><span class=\"diagram-pill\">संगठन / ऐप</span><small class=\"diagram-muted\">team override for one app</small><code>context/brand.md</code></div><div class=\"diagram-arrow diagram-muted\" aria-hidden=\"true\">&darr;</div><div class=\"diagram-card\"><span class=\"diagram-pill accent\">Personal</span><small class=\"diagram-muted\">per-user override &mdash; wins</small><code>context/brand.md</code></div><div class=\"diagram-arrow diagram-accent\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-box ok\">Effective <code>context/brand.md</code></div></div>",
...
```an-callout
{ "tone": "info", "body": "कोई पुनरारंभ नहीं, कोई पुनः तैनाती नहीं। `AGENTS.md` हर मोड़ की शुरुआत में पढ़ा जाता है, इसलिए अब आपके द्वारा सहेजा गया संपादन अगले संदेश पर एजेंट के व्यवहार को बदल देता है।" }
```
...
```text
context/company.md              # कंपनी क्या करती है, ICP, उत्पाद, लिंक
context/brand.md                # आवाज, दृश्य पहचान, वर्तनी, निषिद्ध उपयोग
context/messaging.md            # स्थिति, मूल्य सहारा, प्रमाण बिंदु, आपत्तियां
instructions/guardrails.md      # अनुपालन, वृद्धि और अनुमोदन नियम
...
```an-schema title="कार्यस्थान संसाधन मॉडल" summary="एक संसाधन आकार प्रत्येक कार्यक्षेत्र फ़ाइल का समर्थन करता है। रनटाइम इसे पथ और दायरे के आधार पर कुंजीबद्ध करता है और पढ़ने पर प्रभावी मान का समाधान करता है।"
{
  "entities": [
    {
      "id": "resource",
      "name": "workspace resource",
      "note": "उपयोगकर्ता के कार्यक्षेत्र में एक एकल फ़ाइल - निर्देश, कौशल, मेमोरी, एजेंट, MCP कॉन्फ़िगरेशन, या कार्य।",
      "fields": [
        { "name": "path", "type": "string", "note": "Canonical path, e.g. AGENTS.md, skills/<slug>/SKILL.md" },
        { "name": "scope", "type": "workspace | shared | personal", "note": "यह पंक्ति किस स्तर पर रहती है" },
        { "name": "owner", "type": "string", "nullable": true, "note": "u:<email> for personal scope" },
        { "name": "content", "type": "text", "note": "मार्कडाउन / JSON / YAML बॉडी" }
      ]
    }
  ]
}
...
```an-annotated-code title="एक कस्टम एजेंट प्रोफ़ाइल"
{
  ...
  "language": "markdown",
  "code": "---\nname: Design\ndescription: >-\n  Reviews layouts, interaction patterns, and product UX decisions.\nmodel: inherit\ntools: inherit\ndelegate-default: false\n---\n\n# भूमिका\n\nआप एक केंद्रित डिजाइन एजेंट हैं।\n\n## जिम्मेदारियां\n\n- लेआउट और इंटरैक्शन प्रवाह की समीक्षा करें\n- मजबूत दृश्य दिशा का सुझाव दें\n- संक्षिप्त और विचारशील बनें",
  "annotations": [
    { "lines": "2", "label": "@mention handle", "note": "`name` वह है जो `@`-ड्रॉपडाउन में दिखाई देता है और मुख्य एजेंट जिसे सौंपता है।" },
    { "lines": "3-4", "label": "कब प्रत्यायोजित करें", "note": "`description` वह है जिसे ऑर्केस्ट्रेटर यह तय करने के लिए पढ़ता है कि यह प्रोफ़ाइल किसी कार्य के लिए उपयुक्त है।" },
    { "lines": "5", "label": "मॉडल", "note": "`inherit` मुख्य एजेंट के मॉडल का पुन: उपयोग करता है। केवल तभी ओवरराइड करें जब प्रोफ़ाइल को स्पष्ट रूप से किसी भिन्न प्रोफ़ाइल की आवश्यकता हो।" },
    { "lines": "6", "note": "`tools: inherit` अभी के लिए - फ़ील्ड भविष्य की प्रति-एजेंट टूल नीतियों के लिए आरक्षित है।" }
  ]
}
```
...
# डिस्पैच द्वारा प्रबंधित विरासत में मिला कार्यक्षेत्र संदर्भ पढ़ें
...
# एक संसाधन लिखें
pnpm action resource-write --path "notes/meeting.md" --content "# मीटिंग नोट्स..."
